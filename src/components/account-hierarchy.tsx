...
interface AccountHierarchyProps {
  profile: FullProfile // The parent profile containing subaccounts
...
export function AccountHierarchy({ profile, isOwnProfile }: AccountHierarchyProps) {
  ...
  const handleAddSubidentity = () => {
    navigate(`/register?flow=subidentity&parentId=${profile.address}`)
  }

  const handleEditSubidentity = (subAddress: string) => {
    navigate(`/register?editId=${subAddress}&flow=subidentity&parentId=${profile.address}`)
  }
        ...
        profile.subaccounts.map((account) => (
          ...
          >
            <CardContent className="p-2.5 sm:p-3">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-white truncate" title={account.identity.info.display || 'No display name'}>
                      {account.identity.info.display || 'Anonymous'}
                    </p>
                    <p className="text-xs text-gray-400 font-mono truncate" title={account.address}>
                      {shortenAddress(account.address, 5, 5)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-1.5 sm:space-x-2 self-end sm:self-center flex-shrink-0">
                  {isOwnProfile && (
                    <Button
                      ...
                      onClick={() => handleEditSubidentity(account.address)}
                      aria-label={`Edit ${account.identity.info.display || 'account'}`}
                      title={`Edit ${account.identity.info.display || 'account'}`}
                    >
                      <Edit2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                    </Button>
                  )}
                  <Link to={`/profile/${account.network}/${account.address}`}>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="text-xs h-auto p-1 sm:p-1.5 text-pink-400 hover:bg-pink-500/10 hover:text-pink-300 w-6 h-6 sm:w-7 sm:h-7"
                      aria-label={`View ${account.identity.info.display || 'account'}'s profile`}
                      title={`View ${account.identity.info.display || 'account'}'s profile`}
                    >
              ...
              {account.identity.info.legal && (
                <p className="text-xs text-gray-500 mt-1 sm:mt-1.5 pl-6 sm:pl-7" title={`Legal: ${account.identity.info.legal}`}>
                  Legal: {account.identity.info.legal}
                </p>
              )}
            </CardContent>
          </Card>
