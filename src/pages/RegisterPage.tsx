...
export default function RegisterPage() {
  ...
  const backTo = isEditMode && editingProfileId ? `/profile/${network}/${editingProfileId}` : "/"
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          ...
          {/* View-only: not yet a registrar on this network */}
          {isViewOnly && (
            <div className="py-12 text-center">
              ...
              <p className="text-gray-400 text-sm mb-4">
                ...
                Or test with us for free on Paseo.
              </p>
              <div className="flex gap-3 justify-center">
                <Button variant="default" onClick={() => navigate('/register?network=paseo')}>
                  Register on Paseo
                </Button>
          ...
          {!isViewOnly && !accountStore.address && (
            <div className="py-16 text-center">
              <WalletIcon className="w-10 h-10 text-gray-600 mx-auto mb-4" />
              <h3 className="text-lg font-medium text-white mb-2">Connect Your Wallet</h3>
              <p className="text-gray-400 text-sm mb-6">
                Connect a wallet to register your on-chain identity
              </p>
            ...
            {currentStep === STEP_NUMBERS.connectWallet && (
              <div className="text-center space-y-4">
                {extensions.length > 0 ? (
                  <>
                    {accounts.length <= 0 && (
                      <p className="text-sm text-red-400">No accounts found. Check your wallet extension.</p>
                    )}
                    <Button
                  ...
                  <p className="text-sm text-gray-400">No wallet extensions detected. Install a Polkadot wallet to continue.</p>
                )}
              </div>
            )}
              ...
              <>
                ...
                <div className="mt-6">
                  {identity?.status === IdentityVerificationStatus.IdentityVerified ? (
                    <Button onClick={handleNextStep} className="w-full bg-green-600 hover:bg-green-700">
                      <CheckCircle className="w-4 h-4 mr-2" />
                      Verified — View Profile
                    </Button>
                    ...
                    <div className="py-3 text-sm text-gray-400 rounded-md text-center border border-gray-700/50">
                      {identity?.status === IdentityVerificationStatus.FeePaid ? (
                        <span className="flex items-center justify-center gap-2">
                          <AlertCircle className="w-4 h-4" />
                          Complete verifications above
                        </span>
                      ) : identity?.status === IdentityVerificationStatus.PendingJudgement ? (
                        <span className="flex items-center justify-center gap-2">
                          <Loader2 className="w-4 h-4 animate-spin" />
                          Waiting for registrar...
                        </span>
                      ) : (
                  ...
                  {(identity?.status === IdentityVerificationStatus.PendingJudgement ||
                    ...
                    <Button
                      ...
                      variant="ghost"
                      className="w-full mt-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10"
                    >
                      {isSubmittingIdentity ? "Processing..." : "Cancel & update identity"}
                    </Button>
                  )}
                </div>
              </>
            ...
            {currentStep === STEP_NUMBERS.complete && (
              <div className="text-center py-12">
                ...
                <h2 className="text-xl font-medium text-white mb-2">
                  {isEditMode ? "Identity Updated" : "Registration Complete"}
                </h2>
                <p className="text-gray-400 text-sm mb-6">
                  Your identity is live on {networkDisplayName}.
                </p>
                <Link
                  to={`/profile/${network}/${editingProfileId || walletAddress || "me"}`}
